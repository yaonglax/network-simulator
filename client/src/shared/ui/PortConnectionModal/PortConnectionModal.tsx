...
const PortConnectionModal: React.FC<PortConnectionProps> = ({
    ...
    const handleConnect = () => {
        ...
        try {
            ...
            // Проверяем состояние после соединения
            setTimeout(() => {
                const updatedState = useNetworkStore.getState();
                console.log("После соединения:", {
                    startDevice: updatedState.devices[deviceIdStart],
                    endDevice: updatedState.devices[deviceIdEnd]
                });
            }, 100);
                        ...
                        {startDevicePorts.length > 0 ? (
                            startDevicePorts.map((port) => (
                                <MenuItem key={port.id} value={port.id}>
                                    {port.name || port.id} ({port.ip_address})
                                </MenuItem>
                            ))
                        ) : (
